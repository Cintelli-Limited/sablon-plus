...
class ProcessorDocumentTest < SablonPlus::TestCase
  ...
  def setup
    super
    @processor = SablonPlus::Processor::Document
  ...
  def test_simple_field_with_styling_replacement
    result = process(snippet("simple_field_with_styling"), {"system_name" => "SablonPlus 1 million"})

    assert_equal "Generated by SablonPlus 1 million", text(result)
    assert_xml_equal <<-document, result
    <w:p>
...
<w:tbl>
  ...
  def test_loop_with_missing_end_raises_error
    e = assert_raises SablonPlus::TemplateError do
  ...
  def test_image_replacement
    ...
    image = SablonPlus.content(:image, base_path + "fixtures/images/r2d2.jpg")
  ...
  def process(document, context)
    env = SablonPlus::Environment.new(MockTemplate.new, context)
