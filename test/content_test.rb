...
class ContentTest < SablonPlus::TestCase
  def test_can_build_content_objects
    content = SablonPlus.content(:string, "a string")
    assert_instance_of SablonPlus::Content::String, content
  end

  def test_raises_error_when_building_non_registered_type
    e = assert_raises ArgumentError do
      SablonPlus.content :nope, "this should not work"
    end
    assert_equal "Could not find SablonPlus content type with id 'nope'", e.message
  end

  def test_wraps_string_objects
    content = SablonPlus::Content.wrap(67)
    assert_instance_of SablonPlus::Content::String, content
    assert_equal "67", content.string
  end

  def test_raises_an_error_if_no_wrapping_type_was_found
    SablonPlus::Content.remove SablonPlus::Content::String

    e = assert_raises ArgumentError do
      SablonPlus::Content.wrap(43)
    end
    assert_equal "Could not find SablonPlus content type to wrap 43", e.message
  ensure
    SablonPlus::Content.register SablonPlus::Content::String
  end

  def test_does_not_wrap_content_objects
    original_content = SablonPlus.content(:word_ml, "<w:p></w:p>")
    content = SablonPlus::Content.wrap(original_content)
    assert_instance_of SablonPlus::Content::WordML, content
    assert_equal original_content.object_id, content.object_id
  end
end

class CustomContentTest < SablonPlus::TestCase
  ...
  def setup
    SablonPlus::Content.register MyContent
  end

  def teardown
    SablonPlus::Content.remove MyContent
  end

  def test_can_build_custom_content
    content = SablonPlus.content(:custom, 42)
    assert_instance_of MyContent, content
  end

  def test_wraps_custom_content
    content = SablonPlus::Content.wrap(31)
    assert_instance_of MyContent, content
    assert_equal 31, content.numeric
  end
end

class ContentStringTest < SablonPlus::TestCase
  include XmlContentTestSetup

  def test_single_line_string
    SablonPlus.content(:string, 'a normal string').append_to @paragraph, @node, @env
  ...
  def test_numeric_string
    SablonPlus.content(:string, 42).append_to @paragraph, @node, @env
  ...
  def test_blank_string
    SablonPlus.content(:string, '').append_to @paragraph, @node, @env

    assert_xml_equal @template_text, @document
  end
end

class ContentWordMLTest < SablonPlus::TestCase
  include XmlContentTestSetup
  ...
  def test_blank_word_ml
    ...
    SablonPlus.content(:word_ml, '').append_to @paragraph, @node, @env
  ...
  def test_plain_text_word_ml
    ...
    # gets replaced.
    SablonPlus.content(:word_ml, "test").append_to @paragraph, @node, @env
  ...
  def test_inserts_paragraph_word_ml_into_the_document
    ...
    SablonPlus.content(:word_ml, @word_ml).append_to @paragraph, @node, @env
...
class ContentImageTest < SablonPlus::TestCase
  def setup
    base_path = Pathname.new(File.expand_path('../', __FILE__))
    fixture_dir = base_path.join('fixtures')
    @image_path = fixture_dir.join('images', 'r2d2.jpg')
    @expected = SablonPlus::Content::Image.new(@image_path.to_s)
  end
  ...
  def test_wraps_readable_object_that_responds_to_filename
    ...
    tested = SablonPlus.content(:image, readable)
    assert_equal @expected, tested
  end
  ...
  def test_raises_error_when_no_filename
    ...
    assert_raises ArgumentError do
      SablonPlus.content(:image, data)
    end
  end

  def test_width_conversion
    img = SablonPlus.content(:image, @image_path.to_s, properties: {width: '1.0cm'})
    assert_equal 360000, img.width
    assert_nil img.height
  end

  def test_height_conversion
    img = SablonPlus.content(:image, @image_path.to_s, properties: {height: '1.0in'})
    assert_nil img.width
    assert_equal 914400, img.height
  end

  def test_invalid_unit_conversion
    img = SablonPlus.content(:image, @image_path.to_s, properties: {width: '100px'})
    assert_raises ArgumentError do
      img.width
    end
  end
